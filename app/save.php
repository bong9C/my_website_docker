...
// POST로 넘어온 교수님 이름
$professor_name = trim($_POST['professor_name'] ?? '일반자료');
...
try {
    ...
    // 2. 게시글 데이터베이스 저장 (professor_name 컬럼 사용)
    $stmt = $pdo->prepare("INSERT INTO posts (name, title, content, professor_name) VALUES (?, ?, ?, ?)");
    $stmt->execute([$name, $title, $content, $professor_name]);
